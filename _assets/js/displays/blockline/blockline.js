 ...
 */
export function BlockLine(settings) {
  ...
  try {
    ...
    if (CONTAINER) {
      ...
      var views = Builder.build({
        ...
        types: SETTINGS.types,}); // the types of blockframes contained in the source SVG for display

        COLOR = views[0]; // the color instance of the blockline
        BW = views[1]; // the black-and-white instance of the blockline

        var buildings = BW.node.querySelectorAll(".building");
        ...
        // tl.to(stories, {duration: 1, opacity: 1, onUpdate: updateView, stagger: .25});
...
function updateView() {
  ...
  if (SETTINGS.type == "element") {
    ...
    CONTAINER.style.backgroundImage = `url("data:image/svg+xml;charset=UTF-8,${escape(BW.svg())}")`
  }
